...
# Podstawowe pakiety ----
sudo sed 's/^#TotalDownload/TotalDownload/' -i /etc/pacman.conf #ogólny postęp pobierania w Pacmanie zamiast cząstkowego
sudo sed 's/^#Color/Color/' -i /etc/pacman.conf #kolory w pacmanie
sudo pacman-mirrors --fasttrack
sudo pacman -Syyu
sudo pacman -Syu
sudo pacman -S base-devel fish python-pip python-wxpython jre-openjdk libreoffice-fresh gcc-fortran r yay gimp flatpak caprine calibre flameshot gparted htop qbittorrent vlc etcher pdftk virtualbox aspell hspell libvoikko --needed
